...
class QConv(nn.Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=True,
                momentum = 0.1,
                num_bit = 1, QInput = True, bSetQ = True):
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, num_blocks, num_classes=10, num_bit=1, a_sig=0, a_temp=0):
...
def resnet20_DAQ(pretrained=True, **kwargs):
