...
class QuaOp(object):
    ...
    def __init__(self, model, QW_biases, QW_values=[]):
        ...
        for m in model.modules():
            if isinstance(m, nn.Conv2d) or isinstance(m, nn.Linear):
                ...
                if index in self.bin_range:
                    ...
                    self.saved_params.append(tmp) ## saved_params?
                    self.target_modules.append(m.weight) ## target_modules?
        ...
        self.n = len(self.QW_values) - 1 # 2
        self.threshold = self.QW_values[-1] * 5 / 4.0 # threshold * 1.25
        ...
        for i in range(self.n):
            ...
            self.scales.append(gap) # [gap1, gap2]
            offset += gap # offset: 2
        self.offset = offset / 2. # offset: 1
